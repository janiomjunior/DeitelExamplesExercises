...
class KnightTour {
	...
	public static void solveKnightTourProblem(int chessboardSize) {	
	...
	public static boolean solveProblem(int[][] visited, int moveCount, int x, int y) {
		...
		for (int i = 0; i < xMoves.length; ++i) {
			...
			if ( isValidMove(visited, nextX, nextY) && visited[nextX][nextY] == 0) {
				...
				if ( solveProblem(visited, moveCount + 1, nextX, nextY) ) {
					System.out.print(nextX + ","  + nextY + ":" + (moveCount + 1) + " ");
					return true; 
